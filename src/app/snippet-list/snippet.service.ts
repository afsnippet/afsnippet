...
export class SnippetService {
  ...
  constructor(public firestore: AngularFirestore,
    ...
    private auth: AuthService) {
      ...
      // Assign user to the snippetdata
    ...
    this.snippets = this.usersCollection.snapshotChanges().pipe(map(changes => {
  ...
  updateSnippet(snippet: Snippet) {
    ...
    this.snippetDoc = this.firestore.doc('snippets/' + snippet.uid);
    this.snippetDoc.update(snippetToSave).then(
